...
def chk(): 
  ...
  cetak(panel(f"""[bold white]   
     ...
     ███████╗██║╚██████╗███████╗██║ ╚████║███████║██║
     ╚══════╝╚═╝ ╚═════╝╚══════╝╚═╝  ╚═══╝╚══════╝╚═╝
""",width=62,title=f"",style=f"bold blue"))
  uuid = str(os.geteuid()) + str(os.getlogin()) 
  key = "1".join(uuid) 
  print("\x1b[37;1m YOUR KEY :\x1b[91m "+key) 
  try: 
    httpCaht = requests.get("https://github.com/MANZXSTORE/Licensi/blob/main/manz.txt").text 
    if key in httpCaht: 
      cetak(panel("[bold cyan] KEY KAMU SUDAH DI KONFIRMASI , GUNAKAN DENGAN BIJAK",width=62,title=f"[[green] MANZ X STORE [/]]",padding=(0,2),style=f"bold blue"));time.sleep(5) 
      msg = str(os.geteuid()) 
      time.sleep(3) 
      pass 
    else: 
      cetak(panel('[bold red] KEY KAMU BELUM DI KONFIRMASI , SEDANG DI ARAHKAN KE WHATSAPP',width=62,title=f"[[green] MANZ X STORE [/]]",padding=(0,2),style=f"bold blue"))
      cetak(panel(f"""[bold cyan]          35k Aktif Selama 1 Minggu\n          70k Aktif Selama 1 Bulan\n          150k Aktif selama 1 Tahun""",width=62,padding=(0,2),title=f"",style=f"bold blue"));time.sleep(3) 
      os.system('xdg-open https://wa.me/6289509290349?text=TOLONG+KONFIRMAS+KEY+SAYA+BANG+MANZ+'+key)
      time.sleep(3) 
      sys.exit() 
  except: 
    sys.exit() 
...
def display_license_menu_banner(console_width): 
    ...
    [bold cyan] 
                ...
                ▒█▄▄█ ▄█▄ ▒█▄▄█ ▒█▄▄▄ ▒█░░▀█ ▒█▄▄▄█ ▒█▄▄▄ 
    """ 
    console.print(panel(banner_content, title="[ MENU LISENSI ]", border_style="blue")) 
...
class MAIN:
   ...
   def Me(self):
       self.MyRich('''[bold blue] 
╔╦╗╔═╗╔╗╔╔═╗  ─┐ ┬  ╔═╗╔╦╗╔═╗╦═╗╔═╗ [white]| MURHACK BY MANZ[bold blue]
║║║╠═╣║║║╔═╝  ┌┴┬┘  ╚═╗ ║ ║ ║╠╦╝║╣  [white]| SCRIPT TERBATAS[bold blue]
╩ ╩╩ ╩╝╚╝╚═╝  ┴ └─  ╚═╝ ╩ ╚═╝╩╚═╚═╝ [white]| GUNAKAN DENGAN BAIK[bold blue]
''')
